...
function loadFromLocalStorage() {
  try {
    const saved = localStorage.getItem('riskRegisterData');
    if (saved) {
    ...
    console.error('Failed to load risks from local storage', e);
  }
  return false;
}
...
function getNextId()  { return risks.length > 0 ? Math.max(...risks.map(r => r.id)) + 1 : 1; }
function getNextKey() { return risks.length > 0 ? Math.max(...risks.map(r => parseInt(r.key.replace('RR-',''), 10) || 0)) + 1 : 1; }
